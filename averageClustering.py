...
def find_closest_clusters(clusters):
    min_avg_distance, closest_pair = float('inf'), [0, 1]
    for i in range(len(clusters)):
        for j in range(i + 1, len(clusters)):
            avg_distance = sum(calculate_distance(a, b) for a in clusters[i].data_points for b in clusters[j].data_points) / (len(clusters[i].data_points) * len(clusters[j].data_points))
            if avg_distance < min_avg_distance:
                min_avg_distance, closest_pair = avg_distance, [i, j]
    return closest_pair
...
def hierarchical_clustering_average(num_points, data_points):
...
if __name__ == "__main__":
    ...
    hierarchical_clustering_average(num_points, data_points)
